...
class OS_lab():
    ...
    def __init__(self, 
                 ...
                 bat_inicial_master=91,
                 bat_inicial_slave=91,
                 debug=False
                 ):
        ...
        self.Bess_master = Bess(bat_inicial_master, debug=self.debug)
        self.Bess_slave = Bess(bat_inicial_slave, debug=self.debug)
        #estat inicial bateries
        self.bat_ini_master = bat_inicial_master
        self.bat_ini_slave = bat_inicial_slave
    ...
    def cost(self,x):
        ...
        m=[]
        num_bat=2
        lim_bat = num_bat*24 #Limite donde empiezan los datos de la siguiente batería.
        for i in range(0,lim_bat):
            ...
            elif i in range(24,lim_bat):
                powers_2.append( self.Bess_slave.control(method=m,load = self.ec[i-24], production = self.sp[i-24]))
        
        #simulem bateria.
        kwh_Bess_1, SoC_Bess_1 = self.Bess_master.simula(powers_1[0:])
        kwh_Bess_2, SoC_Bess_2 = self.Bess_slave.simula(powers_2[0:])
        
        #calculem que gastem /injectem a xarxa
        valans_energetic = self.ec - self.sp  + kwh_Bess_1 + kwh_Bess_2
        ...
        bats_1 = (SoC_Bess_1[-1] - self.bat_ini_master) * self.Bess_master.bat_max * self.prices_venta.mean()
        bats_2 = (SoC_Bess_2[-1] - self.bat_ini_slave) * self.Bess_slave.bat_max * self.prices_venta.mean()
        
        #funcio de cost
        preu = sum(consumit * self.prices) - sum(injectat * self.prices_venta) + bats_1 + bats_2       
        return preu
